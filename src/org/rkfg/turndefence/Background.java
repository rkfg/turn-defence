...
public class Background extends Actor {
    ...
    public Background() {
        ...
        mBackgroundTexture = TurnDefence.myAssetManager.get("gfx/stars_1.jpg",
                Texture.class);
        mSmokeTexture = TurnDefence.myAssetManager.get("gfx/smoke.png", Texture.class);
    ...
    public void act(float delta) {
        ...
        for (i = 0; i < (int) TurnDefence.BFHEIGHT / 30; i++) {
            ...
            if (mSmokeParticles[i].z == 0.0f) {
                mSmokeParticles[i].x = TurnDefence.BFWIDTH / 2 + TurnDefence.myRandom.nextFloat()
                        * 20.0f - 10.0f - 32.0f;
                mSmokeParticles[i].y = TurnDefence.BFHEIGHT - TurnDefence.myRandom.nextFloat()
                        * 20.0f - i * 30 - 40;
        ...
        if (TurnDefence.TurnSwitchProcess > 0) { // move
            // camera
            if (TurnDefence.TurnSwitchProcess == TurnDefence.TURNSWITCH) { // init movement
                if (TurnDefence.Turn == 0)
                    camTargetX = TurnDefence.screenWidth / 2;
                else
                    camTargetX = TurnDefence.BFWIDTH - TurnDefence.screenWidth / 2;
            ...
            if (TurnDefence.TurnSwitchProcess > TurnDefence.TURNSWITCH / 2)
                interpolatedDelta = delta
                        * (TurnDefence.TURNSWITCH - TurnDefence.TurnSwitchProcess) / TurnDefence.TURNSWITCH
                        * 4.0f;
            else
                interpolatedDelta = delta * TurnDefence.TurnSwitchProcess / TurnDefence.TURNSWITCH
                        * 4.0f;
        ...
        if (TurnDefence.TurnProcess > 0 && TurnDefence.TurnSwitchProcess == 0) { // move
                                                           // units
                ...
                if (TurnDefence.PlayTime < TurnDefence.PresentPlayTime
                    ...
                    try {
                        TurnDefence.UnitsGroup[1 - TurnDefence.Turn]
                                .addActor(TimeMachine.respawn(TurnDefence.PlayTime,
                                        Meteor.class, 1 - TurnDefence.Turn));
                    } catch (NotRespawnable e) {
                        Gdx.app.log("Time conflict",
                                "Error respawning a meteor, mPlayTime: "
                                        + TurnDefence.PlayTime + " TurnDefence.Turn: " + TurnDefence.Turn
                                        + " mPresentPlayTime: "
                    ...
                    switch (mMeteorSize) {
                    case 0:
                        meteor = new Meteor(1 - TurnDefence.Turn, 1, TurnDefence.MeteorLife * 2);
                        break;
                    case 1:
                    case 2:
                        meteor = new Meteor(1 - TurnDefence.Turn, 2, TurnDefence.MeteorLife);
                        break;
                    default:
                        meteor = new Meteor(1 - TurnDefence.Turn, 3, TurnDefence.MeteorLife / 2);
                        break;
                    }
                    TurnDefence.UnitsGroup[1 - TurnDefence.Turn].addActor(meteor);
                }
    ...
    public void touchDragged(float x, float y, int pointer) {
        ...
        if (camera.position.x + deltaX > TurnDefence.BFWIDTH - TurnDefence.screenWidth / 2)
            deltaX = TurnDefence.BFWIDTH - TurnDefence.screenWidth / 2 - camera.position.x;
        if (camera.position.y + deltaY < TurnDefence.screenHeight / 2
                + (TurnDefence.screenHeight - TurnDefence.BFHEIGHT))
            deltaY = TurnDefence.screenHeight / 2 + (TurnDefence.screenHeight - TurnDefence.BFHEIGHT)
                    - camera.position.y;
        if (camera.position.y + deltaY > TurnDefence.screenHeight / 2)
